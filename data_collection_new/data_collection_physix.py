...
class IsaacSim():
    ...
    def create_ball(self):
        ...
        if big == 3:
            self.ball_radius = round(random.uniform(0.01, 0.02), 4)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for i in range(num_envs):
            ...
            self.bowl_x = 0.59
            bowl_pose.p = gymapi.Vec3(self.bowl_x, -0.11 , 0.02)
    ...
    def offset_generate(self):
        ...
        self.D_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(3, 5))) 
        self.L_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 3)))
        self.R_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 3))) 
        self.B_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 3))) 
    ...
    def data_collection(self):
        ...
        while not self.gym.query_viewer_has_closed(self.viewer):
            ...
            if all(round >= (self.collect_time) for round in self.round) and go_to_final == False:
                ...
                # print(eepose.shape, ball_pcd.shape, spillage_amount.shape)
    ...
    def get_info(self, env_index):
        ...
        if self.round[env_index] < self.collect_time :
            ...
            nor_top_depth = self.normalize_depth(top_depth_image)
            ...
            real_depth = np.load("real_depth.npy")
    
            real_bowl_mask = np.load("ref_bowl_mask.npy").astype(int)
            real_food_mask = np.load("real_food_mask.npy").astype(int)
            
            '''
            fig, axes = plt.subplots(1, 2, figsize=(12, 6))  # Create a figure with 1 row and 2 columns

            # Display the first image
            axes[0].imshow(real_depth, cmap='viridis')  # Adjust cmap if needed
            axes[0].set_title("Real Depth")
            axes[0].axis('off')  # Turn off axis

            # Display the second image
            axes[1].imshow(back_depth_image, cmap='viridis')  # Adjust cmap if needed
            axes[1].set_title("Back Depth Image")
            ...
            # ball_pcd = self.depth_to_point_cloud(nor_back_depth, back_seg_image, object_id = 2)

            top_seg_image[:, 700:, ...] = 0

            back_bowl_pcd = self.depth_to_point_cloud(back_depth_image, back_seg_image, object_id = 4)
            front_bowl_pcd = self.depth_to_point_cloud(top_depth_image, top_seg_image, object_id = 4)
            real_bowl_pcd = self.depth_to_point_cloud(real_depth, real_bowl_mask, object_id = 1)
            real_food_pcd = self.depth_to_point_cloud(real_depth, real_food_mask, object_id = 1)


     
            back_tool_pcd = self.depth_to_point_cloud(back_depth_image, back_seg_image, object_id = 1)
            front_tool_pcd = self.depth_to_point_cloud(top_depth_image, top_seg_image, object_id = 1)

            
            gt_front = np.load("./real_cam_pose/front_cam2base.npy")
            gt_back = np.load("./real_cam_pose/back_cam2base.npy")
            

            front_points_world = self.transform_to_world(front_bowl_pcd[:, :3], gt_front)
            back_points_world = self.transform_to_world(back_bowl_pcd[:, :3], gt_back)
            real_points_world = self.transform_to_world(real_bowl_pcd[:, :3], gt_back)

            
            
            back_tool_world = self.transform_to_world(back_tool_pcd[:, :3], gt_back)
            front_tool_world = self.transform_to_world(front_tool_pcd[:, :3], gt_front)

            back_food_world = self.transform_to_world(real_food_pcd[:, :3], gt_back)

            # np.save("init_spoon_pcd.npy", front_tool_world[:, :3])
            # exit()
            ...
            # back_points_world = self.process_raw(back_points_world)
            # front_points_world = self.process_raw(front_points_world)
            # real_points_world = self.process_raw(real_points_world)


            object_seg = np.full((front_points_world.shape[0], 1), 1)
            front_bowl_world = np.hstack((front_points_world, object_seg))

            object_seg = np.full((back_points_world.shape[0], 1), 4)
            back_bowl_world = np.hstack((back_points_world, object_seg))

            # np.save("ref_sim_bowl.npy", back_bowl_world)
            # exit()


            object_seg = np.full((real_points_world.shape[0], 1), 3)
            real_bowl_world = np.hstack((real_points_world, object_seg))

            

            object_seg = np.full((back_tool_world.shape[0], 1), 4)
            back_tool_world = np.hstack((back_tool_world, object_seg))

            object_seg = np.full((front_tool_world.shape[0], 1), 0)
            front_tool_world = np.hstack((front_tool_world, object_seg))

            object_seg = np.full((trans_tool.shape[0], 1), 2)
            trans_tool = np.hstack((trans_tool, object_seg))

            object_seg = np.full((back_food_world.shape[0], 1), 1)
            back_food_world= np.hstack((back_food_world, object_seg))

            ref_bowl = np.load("ref_sim_bowl.npy")
            # object_seg = np.full((ref_bowl.shape[0], 1), 4)
            # ref_bowl = np.hstack((ref_bowl, object_seg))
            
            # real_pcd = self.transform_to_camera(real_bowl[:, :3], gt_back)
            
          
            # all_pcd = np.concatenate((ball_pcd, self.bowl_pcd, tool_pcd), axis=0)
            # check_pcd = np.concatenate((front_tool_world, trans_tool, back_bowl_world, real_bowl_world, back_food_world), axis=0)
            # check_pcd = np.concatenate(( back_bowl_world, real_bowl_world, back_food_world, trans_tool, back_tool_world), axis=0)

            # back_bowl_world[:, 2] -= 0.03
            # trans_tool[:, 2] -= 0.02
            check_pcd = np.concatenate((ref_bowl, back_food_world, trans_tool, front_tool_world, real_bowl_world), axis=0)
           
            
            self.check_pcd_color(check_pcd, sim_cor, self.mean_eepose_qua[0])
            exit()
         
            # self.check_pcd_color(check_pcd)
    ...
    def process_raw(self, pcd_points):

        pcd = o3d.geometry.PointCloud()
        pcd.points = o3d.utility.Vector3dVector(pcd_points)
        down_pcd = pcd.uniform_down_sample(every_k_points=2)
        ...
        centroid = [ 0.5818854 , -0.10671745 , 0.05186225]
        m = 0.09
    ...
    def depth_to_point_cloud(
        ...
        # intrinsic_matrix = np.load("/home/hcis-s22/benyang/scoop-env/data_collection_new/real_cam_pose/orbbec_intrinsic.npy")
        intrinsic = self.compute_camera_intrinsics_matrix(1280, 960, 65)
        # intrinsic = torch.from_numpy(intrinsic.astype(np.float32)).to(device)
    ...
    def check_pcd_color(self, pcd, sim_coord_data = None, real_coord_data = None):
...
if __name__ == "__main__":
    ...
    # [60:150]
    mean_traj = np.load("./sample_trial/mean_traj.npy")[index:155]
    # mean_traj = np.load("./sample_trial/sym_05/joint_states.npy")[index:155]
    mean_eepsoe_euler = np.load("./sample_trial/sym_05/ee_pose_euler.npy")[index:155]
    mean_eepsoe_qua = np.load("./sample_trial/sym_05/ee_pose_qua.npy")[index:155]
