...
class DiffusionPolicy(nn.Module):
    ...
    def goal_objective(self, traj, goal_pos):
        ...
        goal_gradient = torch.clip(guided_grad[..., 1], min=-0.2, max=0.2)
        ...
        if start_guidance:
            ...
            for t in scheduler.timesteps:
                ...
                with torch.enable_grad():
                    ...
                    # 1. goal contraint
                    goal_gradient = self.goal_objective(model_output, goal_position)

                    # 2. spillage constraint
                    # spillage_gradient = self.spillage_objective(obs_in, model_output, spillage_classifier)

                    # 3. amount constraint
                    # amount_gradient = self.amount_objective(obs_in, model_output, amount_predictor)        

                # combine  

                # update
                model_output[..., 1] -= self.guided_weight * self.guided_alpha * goal_gradient
