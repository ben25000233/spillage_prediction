...
import time
from rgbd_ee_model import SingleObEncoder, DiffusionPolicy, EMAModel, RotationTransformer
...
class LfD():
    def __init__(self):
        ...
        checkpoint = torch.load('epoch=350.pth', map_location="cuda:0")
    ...
    def run_model(self, image_list, depth_list, eepose_list):
        ...
        with torch.no_grad():
            ...
            rot_6d = rotation_transformer_forward.forward(eepose_list[:, 3:])
            ee_in = np.concatenate((eepose_list[:, :3], rot_6d), -1)
            ee_in = torch.unsqueeze(torch.tensor(ee_in), 0).to('cuda:0', dtype=torch.float32)
