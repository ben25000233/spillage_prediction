        ...
        if start_guidance:
            ...
            for t in scheduler.timesteps:
                ...
                if t<=30:
                    ...
                    dist_in_threshold = torch.where(dist<0.04)
                    ...
                    with torch.enable_grad():
                            ...
                            if self.trigger:
                                goal_gradient = torch.clip(self.local_goal_objective(clean_traj, goal_position), min=-0.2, max=0.2)   
                                # update
                                traj_clone = traj_next.clone()
                                traj_clone[..., :2] -= 1. * goal_gradient[..., :2]
                                traj_next = traj_clone 
                            else:
                                goal_gradient = torch.clip(self.global_goal_objective(clean_traj, goal_position), min=-0.2, max=0.2)
                                # update
                                traj_clone = traj_next.clone()
                                traj_clone[..., :2] -= 1. * goal_gradient[..., :2]
                                traj_next = traj_clone
