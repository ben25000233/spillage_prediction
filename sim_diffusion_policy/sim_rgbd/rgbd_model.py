            ...
            for t in scheduler.timesteps:
                ...
                if t<=50:   
                    ...
                    with torch.enable_grad():
                        ...
                        # traj_clone[..., 1:3] += grad_weight * guided_grad[..., 1:3]
                        # traj_clone[..., 3:] += grad_weight * guided_grad[..., 3:]

                        traj_clone += grad_weight * guided_grad
            ...
            for i in range(opt_step):
                with torch.enable_grad():
                    ...
                    if self.trigger:
                        ...
                        traj_clone[..., 1:3] -= opt_grad_weight * guided_grad[..., 1:3]
                        traj = traj_clone
                
                # show_trajectory(ori_traj = ori_traj[:,self.start:self.end-1], opt_traj = traj[:,self.start:self.end-1])
...
def show_trajectory(ori_traj=None, guided_traj=None, opt_traj=None):
    ...
    if ori_traj is not None:
        ...
        ax.scatter(y_ori[-1], z_ori[-1], c='red', s=100, 
                   label='Original End Point', edgecolor='black', facecolors='none')
    ...
    if guided_traj is not None:
        ...
        ax.scatter(y_guided[-1], z_guided[-1], c='blue', s=100, 
                   label='Guided End Point', edgecolor='black', facecolors='none')
    ...
    if opt_traj is not None:
        ...
        ax.scatter(y_opt[-1], z_opt[-1], c='green', s=100, 
                   label='Optimized End Point', edgecolor='black', facecolors='none')

    # Set the range for both axes
    # ax.set_xlim(-1, 1)  # Set y-axis range
    # ax.set_ylim(-1, 1)  # Set z-axis range
