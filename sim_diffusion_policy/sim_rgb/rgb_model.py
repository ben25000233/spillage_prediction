...
class SingleObEncoder(nn.Module):
    def __init__(self, cfg):
        ...
        self.obs_encoder.conv1 = nn.Conv2d(3, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        self.obs_encoder = replace_bn_with_gn(self.obs_encoder)
        input_image_shape = (3, 240, 320)
    ...
    def forward(self, inputs):
        ...
        for t in range(ob_front.shape[1]):
            ...
            rgb_image_t = ob_front[:, t, :3, :, :]  # (batch_size, C, W, H)
            # ee_pose_t = ee_pose[:, t, :]  # (batch_size, action_dimension)

            obs_feature = self.obs_encoder(rgb_image_t)  # (batch_size, 512)
...
class DiffusionPolicy(nn.Module):
    ...
    def predict_action(self, inputs):
        ...
        # obs, ee_pose, goal_position, spillage_classifier, start_guidance = inputs, False, False, False
        obs, ee_pose = inputs
        start_guidance = goal_position = spillage_classifier = False
        ...
        # depth_image = obs[0,0,3,:,:].cpu()
        # rgb_image = obs[0,0,:3,:,:].cpu()

        # plt.imshow(depth_image.reshape(240, 320, 1))
        # plt.show()
        # plt.imshow(rgb_image.reshape(240, 320, 3))
        # plt.show()
        # exit()
