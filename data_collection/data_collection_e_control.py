...
class IsaacSim():
    ...
    def create_ball(self):
        self.ball_radius = round(random.uniform(0.003, 0.007), 4)
        self.ball_mass = round(random.uniform(0.001, 0.02), 4)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.split_all_pcd_list = [[] for _ in range(self.num_envs)]
        ...
        for i in range(num_envs):
            ...
            self.bowl_x = 0.59
            bowl_pose.p = gymapi.Vec3(self.bowl_x, -0.11 , 0.05)
    ...
    def offset_generate(self):
        ...
        self.L_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 10)))
        self.R_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(20, 20))) 
        self.B_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(30, 30))) 
        self.F_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 10))) 
    ...
    def apply_offset(self, index):
        ...
        #     offset +=self.action_space.get("left")/1000
        
        if index in self.R_index :
            offset += self.action_space.get("right")/500

        # if index in self.D_index :
        #     offset += self.action_space.get("down")/1000

        # if index in self.F_index :
        #     offset += self.action_space.get("forward")/1000

        if index in self.B_index :
            offset += self.action_space.get("backward")/1000

        # if index in self.U_index :
        #     offset += self.action_space.get("up")/1000
        ...
        while not self.gym.query_viewer_has_closed(self.viewer):
            ...
            if self.frame <= 95 : 
                ...
                if check_final_times == 0:
                    ...
                    # if scoop_amount[-1] > 0:
                    #     with h5py.File(f'{f"/media/hcis-s25/data/sim_spillage_dataset/last_frame/time_{self.count}"}.h5', 'w') as h5file:
                ...
                if self.frame % self.slow_time == 1:
                    ...
                    # self.mean_eepose_euler[self.dpose_index[0]:] += offset

                
                isaac_eepose, _ = self.get_current_qua()
                quat_wxyz = isaac_eepose[3:]
                quat_xyzw = [quat_wxyz[1], quat_wxyz[2], quat_wxyz[3], quat_wxyz[0]]
                rotation = Rot.from_quat(quat_xyzw)
                euler_angles = rotation.as_euler('xyz')

                current_euler = np.concatenate((isaac_eepose[:3], euler_angles), axis=0)
                
 
                dpose = self.mean_eepose_euler[self.dpose_index[0]] - current_euler
                dpose[3] = -dpose[4]
                dpose[4:] = 0
            ...
            for i in range(self.num_envs):
                ...
                #     self.get_info(i, self.dpose_index[0])
            ...
            if all(round >= (self.collect_time) for round in self.round) and go_to_final == False:
                split_all_pcd = self.list_to_nparray(self.split_all_pcd_list)
                mix_all_pcd = self.list_to_nparray(self.mix_all_pcd_list)
                eepose = self.list_to_nparray(self.record_ee_pose)
                ...
                data_dict = {
                    'eepose' : eepose[1:],
                    'split_all_pcd' : split_all_pcd[1:],
                    'mix_all_pcd' : mix_all_pcd[1:],
                ...
                # store the data
                with h5py.File(f'{f"/media/hcis-s22/data/sim_spillage_dataset/all_process/time_{self.count}"}.h5', 'w') as h5file:
                    for key, value in data_dict.items():
                        h5file.create_dataset(key, data=value)
    ...
    def get_info(self, env_index, index):
        ...
        if self.round[env_index] < self.collect_time :
            ...
            back_rgb_image = back_rgb_image.reshape(960, 1280, 4)[..., :3]
            # import cv2
            # cv2.imwrite('back_rgb_image.jpg', back_rgb_image)
            # exit()
            ...
            # get bowl pcd
            # back_bowl_pcd = self.depth_to_point_cloud(back_depth_image, back_seg_image, object_id = 4)
            # back_bowl_world = self.transform_to_world(back_bowl_pcd[:, :3], self.gt_back)
            # object_seg = np.full((back_bowl_world.shape[0], 1), 4)
            # back_bowl_world = np.hstack((back_bowl_world, object_seg))
            # np.save("ref_bowl_pcd", back_bowl_world)
            # exit()
            ...
            # real_pcd = np.load("check_pcd.npy")
            # real_pcd[:, 3] = 3
            
            mix_all_pcd = np.concatenate(( trans_tool, back_food_world, self.ref_bowl), axis=0)
            mix_all_pcd = self.align_point_cloud(mix_all_pcd, target_points = 30000)
            mix_all_nor_pcd = self.nor_pcd(mix_all_pcd)
            

            allign_food = self.align_point_cloud(back_food_world)
            split_all_pcd = np.concatenate(( trans_tool, allign_food, self.ref_bowl), axis=0)
            split_all_pcd = self.nor_pcd(split_all_pcd)

            
            # self.check_pcd_color(split_all_pcd, sim_cor, self.mean_eepose_qua[index])
            # exit()

            # all_pcd = np.concatenate((nor_pcd, real_pcd ), axis=0)
            
            sim_cor = np.array([0,0,0,1,0,0,0])
            # self.draw_axes_on_image(back_rgb_image, sim_cor, self.gt_back)
            # self.check_pcd_color(split_all_pcd, sim_cor, self.mean_eepose_qua[index])
            
            # print(sim_cor)
            # self.check_pcd_color(nor_pcd)
            # np.save('sim_nor_pcd', nor_pcd)
            # exit()
            
            self.mix_all_pcd_list[env_index].append(mix_all_nor_pcd)
            self.split_all_pcd_list[env_index].append(split_all_pcd)
            # self.front_seg_list[env_index].append(top_seg_image)

    def draw_axes_on_image(self, image, pose, extrinsic_matrix, axis_length=0):
        """
        Draws 3D coordinate axes on a 2D image.

        Args:
            image (numpy.ndarray): The input camera view image.
            pose (list): The 3D pose [x, y, z, qw, qx, qy, qz].
            intrinsic_matrix (numpy.ndarray): Camera intrinsic matrix (3x3).
            extrinsic_matrix (numpy.ndarray): Camera extrinsic matrix (4x4).
            axis_length (float): Length of the axes in 3D space (default=50).
        
        Returns:
            numpy.ndarray: The image with overlaid axes.
        """
        # Extract rotation (R) and translation (t) from the extrinsic matrix

        import cv2 

        intrinsic_matrix = self.compute_camera_intrinsics_matrix(1280, 960, 65)

        R = extrinsic_matrix[:3, :3]
        t = extrinsic_matrix[:3, 3]

        # Define the 3D axes in world coordinates
        origin = np.array([pose[:3]])  # 3D origin point
        axes = np.array([
        ...
        # Apply pose translation to the axes
        axes_world = origin + axes

        # Combine origin and axes for projection
        points_3d = np.vstack([origin, axes_world])  # Shape: (4, 3)

        # Convert 3D points to homogeneous coordinates
        points_3d_h = np.hstack([points_3d, np.ones((points_3d.shape[0], 1))])  # Shape: (4, 4)

        # Transform 3D points to camera coordinates
        points_cam = (extrinsic_matrix @ points_3d_h.T).T  # Shape: (4, 4)

        # Project to 2D using the intrinsic matrix
        points_2d = intrinsic_matrix @ points_cam[:, :3].T
        points_2d = points_2d[:2, :] / points_2d[2, :]  # Normalize by depth Z
        ...
        origin_2d = tuple(points_2d[0])  # Projected origin
        x_axis_2d = tuple(points_2d[1])  # Projected X-axis
        y_axis_2d = tuple(points_2d[2])  # Projected Y-axis
        z_axis_2d = tuple(points_2d[3])  # Projected Z-axis

        # Draw lines for each axis
        cv2.line(output_image, origin_2d, x_axis_2d, (0, 0, 255), 3)  # Red for X
        cv2.line(output_image, origin_2d, y_axis_2d, (0, 255, 0), 3)  # Green for Y
        cv2.line(output_image, origin_2d, z_axis_2d, (255, 0, 0), 3)  # Blue for Z
    ...
    def check_pcd_color(self, pcd, sim_coord_data, real_coord_data):
        ...
        '''
        # if base_coord_data != None :
        sim_coord = torch.eye(4)  # 4x4 Identity matrix
        sim_coord[0:3, 3] = torch.tensor(sim_coord_data[:3])

        # sim
        rotation = Rot.from_quat([sim_coord_data[4], sim_coord_data[5], sim_coord_data[6], sim_coord_data[3]]) 
        
        rotation_matrix = torch.tensor(rotation.as_matrix(), dtype=torch.float32)
        sim_coord[0:3, 0:3] = torch.tensor(rotation_matrix)
        
        sim_base_coord = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1)
        sim_base_coord.transform(sim_coord)

        # real
        real_coord = torch.eye(4)  # 4x4 Identity matrix
        real_coord[0:3, 3] = torch.tensor(real_coord_data[:3])

        rotation = Rot.from_quat([real_coord_data[4], real_coord_data[5], real_coord_data[6], real_coord_data[3]]) 
        rotation_matrix = torch.tensor(rotation.as_matrix(), dtype=torch.float32)
        real_coord[0:3, 0:3] = torch.tensor(rotation_matrix)
    
        real_base_coord = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1)
        real_base_coord.transform(real_coord)

        # base_coord = o3d.geometry.TriangleMesh.create_coordinate_frame(size=0.1)



        # o3d.visualization.draw_geometries([point_cloud, sim_base_coord, real_base_coord])
    ...
    def get_current_qua(self):
        # get eepose
        ee_pose = self.gym.get_rigid_transform(self.envs[0], self.ee_handles[0])
                    # sim to real : modify z axis 0.1, y = 0.0105
        isaac_eepose = np.array([ee_pose.p.x, ee_pose.p.y, ee_pose.p.z, ee_pose.r.w, ee_pose.r.x, ee_pose.r.y, ee_pose.r.z])
        isaac_quat_rotation_xyzw = np.array([ee_pose.r.x, ee_pose.r.y, ee_pose.r.z, ee_pose.r.w])
        rotation = Rot.from_quat(isaac_quat_rotation_xyzw)
        rotation_matrix = rotation.as_matrix()
        z_rot = rotation_matrix[:, 2]
        ...
        updata_eepose = [ee_pose.p.x, ee_pose.p.y, ee_pose.p.z] + z_rot*0.1
        updata_eepose = np.array([updata_eepose[0], updata_eepose[1], updata_eepose[2], ee_pose.r.w, ee_pose.r.x, ee_pose.r.y, ee_pose.r.z])
        return isaac_eepose, updata_eepose
...
if __name__ == "__main__":
    ...
    # [60:150]
    # mean_traj = np.load("./sample_trial/col_03/joint_states.npy")[0:150]
    # mean_eepsoe_euler = np.load("./sample_trial/col_03/ee_pose_euler.npy")[0:150]
    # mean_eepose_qua = np.load("./sample_trial/col_03/ee_pose_qua.npy")[0:150]

    mean_traj = np.load("./sample_trial/mean_traj.npy")[30:155]
    mean_eepsoe_euler = np.load("./sample_trial/mean_eepose_euler.npy")[30:155]
    mean_eepose_qua = np.load("./sample_trial/mean_eepose_qua.npy")[30:155]

    # trans real eepose to isaac
    update_eulers = []
    for i in range(len(mean_eepose_qua)):
        real_qua = mean_eepose_qua[i]
        real_euler = mean_eepsoe_euler[i]
        ...
        update_euler = np.array([updata_qua_pose[0], updata_qua_pose[1], updata_qua_pose[2], real_euler[3], real_euler[4], real_euler[5]])
        update_eulers.append(update_euler)

    issac = IsaacSim(mean_traj, np.array(update_eulers), mean_eepose_qua)
