...
class IsaacSim():
    def __init__(self,down_trial,scoop_trial, trans_trial, mean_traj, mean_eepose_euler, mean_eepose_qua):
    ...
    def create_sim(self):
        ...
        sim_params.physx.friction_offset_threshold = 0.00001
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.camera_props.horizontal_fov = 51
        ...
        for i in range(num_envs):
            ...
            # #ori : 0.57 
            self.bowl_x =  0.5
            self.bowl_pose = [self.bowl_x, -0.1 , 0]
            bowl_pose.p = gymapi.Vec3(self.bowl_x, -0.1 , 0)
            ...
            rotation_matrix = front_cam_pose[:3, :3]
            ...
            rotation_matrix = back_cam_pose[:3, :3]
    ...
    def offset_generate(self):
        ...
        self.D_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 5)))
        self.L_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 5)))
        self.R_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 5))) 
        self.B_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(2, 5))) 
        self.F_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 5))) 
        self.U_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 5))) 
    ...
    def get_info(self, env_index):
        ...
        if self.round[env_index] < self.collect_time :
            ...
            back_seg_image = self.gym.get_camera_image(self.sim, self.envs[env_index], self.camera_handles[env_index][1],  gymapi.IMAGE_SEGMENTATION)
            nor_back_depth = self.normalize_image(top_depth_image[30:135, 50:165])


            a, b = back_rgb_image.shape
            plt.imshow(back_rgb_image.reshape(a, int(b/4), 4)[:, :, :3])
            plt.show()
            exit()
...
if __name__ == "__main__":
    ...
    down_trial = np.load("./sample_trial/down_trial.npy")[70:]
    scoop_trial = np.load("./sample_trial/scoop_trial.npy")[70:]
    trans_trial = np.load("./sample_trial/trans_trial.npy")[70:]
    ...
    issac = IsaacSim(down_trial, scoop_trial, trans_trial, mean_traj, mean_eepsoe_euler, mean_eepsoe_qua)
