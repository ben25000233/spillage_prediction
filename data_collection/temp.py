...
class IsaacSim():
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        self.nor_all_pcd_list = [[] for _ in range(self.num_envs)]
    ...
    def offset_generate(self):
        ...
        self.U_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(50, 100))) 
        self.D_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 50)))
        self.L_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 50)))
        self.R_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 50))) 
        self.B_index = np.array(random.sample(range(len(self.mean_traj)), random.randint(0, 50))) 
    ...
    def apply_offset(self, index):
        ...
        if index in self.U_index :
            offset += self.control_ik(self.action_space.get("up"))/10
    ...
    def data_collection(self):
        ...
        while not self.gym.query_viewer_has_closed(self.viewer):
            ...
            if all(round >= (self.collect_time) for round in self.round) and go_to_final == False:
                ...
                nor_all_pcd = self.list_to_nparray(self.nor_all_pcd_list)
                eepose = self.list_to_nparray(self.record_ee_pose)
                ...
                data_dict = {
                    ...
                    'nor_tool_ball_bowl_pcd' : nor_all_pcd[1:],
    ...
    def get_info(self, env_index):
        ...
        if self.round[env_index] < self.collect_time :
            ...
            top_seg_image[:, 600:] = 0
            # get food pcd
            # tool_pcd = self.depth_to_point_cloud(top_depth_image, top_seg_image, object_id = 1)
            # tool_world = self.transform_to_world(tool_pcd[:, :3], self.gt_front)
            # object_seg = np.full((tool_world.shape[0], 1), 1)
            # tool_world = np.hstack((tool_world, object_seg))
            ...
            self.nor_all_pcd_list[env_index].append(nor_pcd)
    ...
    def align_point_cloud(self, points, target_points=30000):
...
if __name__ == "__main__":
    ...
    # [60:150]
    mean_traj = np.load("./sample_trial/col_03/joint_states.npy")[35:160]
    mean_eepsoe_euler = np.load("./sample_trial/col_03/ee_pose_euler.npy")[35:160]
    mean_eepsoe_qua = np.load("./sample_trial/col_03/ee_pose_qua.npy")[35:160]
