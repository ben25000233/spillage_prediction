...
class IsaacSim():
    ...
    def create_ball(self):
        ...
        self.ball_amount = random.randint(3, max_num)
    ...
    def data_collection(self):
        ...
        while not self.gym.query_viewer_has_closed(self.viewer):
            ...
            if all(round >= (self.collect_time) for round in self.round):
                ...
                '''
                # store the data
                with h5py.File(f'{f"/media/hcis-s22/data/single_env_dataset/dataset/time_{self.count}"}.h5', 'w') as h5file:
    ...
    def depth_to_point_cloud(self, env_index, depth_image, seg_image, type = 0):
        ...
        #有陷阱，每個env get_camera_view_matrix return 的值都不同, 只能用env0 或是要調
        vinv = np.linalg.inv(np.matrix(self.gym.get_camera_view_matrix(self.sim, self.envs[0], self.camera_handles[0][type])))
        ...
        filter_bowl = np.array([points[j, i] for j in range(height) for i in range(width) if int(seg_image[j, i]) == 4])

        filter_points = np.concatenate((filter_tool, filter_ball, filter_bowl), axis=0)

        filter_seg = np.array([0] * len(filter_tool) + [1] * len(filter_ball) + [2] * len(filter_bowl))
        seg_pcd = np.concatenate((filter_points, filter_seg.reshape(filter_seg.shape[0], 1)), axis = 1)
