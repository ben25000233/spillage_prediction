...
class MultimodalManipulationDataset(Dataset):
    ...
    def _read_data_from_file(self, dataset, idx):
        ...
        if idx == 0:
            # tool_ball_bowl_pcd = np.tile(dataset["tool_ball_bowl_pcd"][0][0] , (2, 1, 1)).astype(np.float32)
            tool_ball_bowl_pcd = np.tile(dataset["tool_ball_bowl_pcd"][0][0] , (look_back_frame, 1, 1)).astype(np.float32)
        
        else : 
            begin_idx = -look_back_frame 
            # tool_ball_bowl_pcd = dataset["tool_ball_bowl_pcd"][idx-1][[begin_idx,-1]].astype(np.float32)
            tool_ball_bowl_pcd = dataset["tool_ball_bowl_pcd"][idx-1][begin_idx:].astype(np.float32)
            
        # future ee_pose and tool_pcd
        future_eepose_num = 8 
        eepose = dataset["eepose"][idx][1: ] #index 0 = current frame  (w, x, y, z)
        # tool_pcd = dataset["tool_pcd"][current_ee_index: target_eepose].astype(np.float32)

        binary_label = dataset["binary_spillage"][idx]
        
        spoon_pose = from_ee_to_spoon(eepose)
...
def from_ee_to_spoon(ee_traj):
    """
    Converts end-effector trajectory in quaternion format (x, y, z, qw, qx, qy, qz) to spoon trajectory.
    
    Args:
        ee_traj (torch.Tensor): Input tensor of shape (B, H, 7) where each row represents (x, y, z, qw, qx, qy, qz).
    
    Returns:
        torch.Tensor: Spoon trajectory tensor of shape (B, H, 9) where each row is (x, y, z, rotation_6d).
    """
    ee_traj = torch.tensor(ee_traj, dtype=torch.float32)
    T_spoon_to_center = get_translation_matrix(0.03, 0, 0.17)  # Transform matrix (4x4)

    spoon_traj = torch.zeros((ee_traj.shape[0], 9))  # (H, 9)
    ...
    for i in range(ee_traj.shape[0]):
        ...
        ee_pose = torch.eye(4)
        ee_pose[0:3, 3] = position
        ee_pose[0:3, 0:3] = ee_rotation_matrix

        # Apply the transformation
        spoon_pose = torch.mm(ee_pose, T_spoon_to_center)

        # Extract spoon position and orientation (in 6D representation)
        spoon_traj[i, 0:3] = spoon_pose[0:3, 3]
        spoon_traj[i, 3:] = matrix_to_rotation_6d(spoon_pose[0:3, 0:3])

    return np.array(spoon_traj)  # (H, 9)
